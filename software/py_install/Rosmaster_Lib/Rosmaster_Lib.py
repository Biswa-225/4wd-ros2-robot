...
"""Low-level serial protocol library for the Yahboom Rosmaster controller.

Most ROS nodes should not talk to this file directly. The driver nodes use it
to send commands to the STM32/Rosmaster board and read back cached sensor data.

Protocol idea:
    - Every command frame starts with 0xFF and the device id.
    - A function byte selects the command type, such as motor, RGB, IMU, etc.
    - Payload values are packed into bytes with struct.pack().
    - A checksum is appended so the board can reject corrupt frames.
    - A background receive thread reads report frames and updates private
      cached values like velocity, IMU, battery, and encoders.

Important for your Nav2 setup:
    diff_driver_X3.py calls set_motor() and reads get_motor_encoder(),
    get_accelerometer_data(), get_gyroscope_data(), and get_motion_data().
"""
...
class Rosmaster(object):
    """Python wrapper around the Rosmaster serial command/report protocol."""

    __uart_state = 0

    def __init__(self, car_type=1, com="/dev/myserial", delay=.002, debug=False):
        # Serial port to the motor controller board.
        # com = "COM30"
        # com="/dev/ttyTHS1"
        # com="/dev/ttyUSB0"
        # com="/dev/ttyAMA0"
        ...
        # Command timing/debug settings.
        self.__delay_time = delay
        self.__debug = debug

        # Frame constants. __COMPLEMENT is used as the checksum seed.
        self.__HEAD = 0xFF
        ...
        # Function codes understood by the controller firmware.
        ...
        # Cached IMU and motion values updated by __parse_data().
        self.__ax = 0.0
        self.__ay = 0.0
        self.__az = 0.0
        self.__gx = 0.0
        ...
        # Cached response values for read-back commands.
        self.__read_id = 0
        self.__read_val = 0
    ...
    def __parse_data(self, ext_type, ext_data):
        """Decode one received report frame and update cached state.

        ext_type tells which kind of payload arrived. ext_data is the payload
        bytes after the function code. The public get_* methods later return
        the cached values set here.
        """
        # print("parse_data:", ext_data, ext_type)
        if ext_type == self.FUNC_REPORT_SPEED:
            # Velocity report uses signed 16-bit integers scaled by 1000.
            # print(ext_data)
        ...
        # (MPU9250)the original gyroscope, accelerometer, magnetometer data
            ...
            # Convert raw MPU9250 integer units into ROS-friendly SI units.
        ...
        # (ICM20948)the original gyroscope, accelerometer, magnetometer data
            ...
            # ICM20948 reports already use simpler /1000 scaling here.
            gyro_ratio = 1 / 1000.0
            self.__gx = struct.unpack('h', bytearray(ext_data[0:2]))[0]*gyro_ratio
            self.__gy = struct.unpack('h', bytearray(ext_data[2:4]))[0]*gyro_ratio
            self.__gz = struct.unpack('h', bytearray(ext_data[4:6]))[0]*gyro_ratio
        ...
        # the attitude Angle of the board
            ...
            # Board-computed roll/pitch/yaw attitude.
        ...
        # Encoder data on all four wheels
            ...
            # Four signed 32-bit encoder counts from the motor controller.
    ...
    def __receive_data(self):
        """Background loop that reads serial frames forever."""
        # 清空缓冲区
        self.ser.flushInput()
        while True:
            head1 = bytearray(self.ser.read())[0]
            if head1 == self.__HEAD:
                # Valid frames begin with two known header bytes.
                ...
                if head2 == self.__DEVICE_ID - 1:
                    ...
                    while len(ext_data) < data_len:
                        ...
                        if len(ext_data) == data_len:
                            # Last byte in this payload is the received checksum.
                            rx_check_num = value
                        else:
                            check_sum = check_sum + value
                    if check_sum % 256 == rx_check_num:
                        # Only parse packets that pass checksum validation.
    ...
    def __request_data(self, function, param=0):
        """Send a request frame asking the board to report a value."""
    ...
    def __arm_convert_value(self, s_id, s_angle):
        """Convert arm servo angle to the pulse range expected by the board."""
        value = -1
        if s_id == 1:
    ...
    def __arm_convert_angle(self, s_id, s_value):
        """Convert arm servo pulse feedback back into an angle."""
        s_angle = -1
        if s_id == 1:
    ...
    def __limit_motor_value(self, value):
        """Clamp motor PWM values to the firmware-supported range."""
        if value == 127:
            return 127
        elif value > 100:
           return 100
    ...
    def create_receive_threading(self):
        """Start the serial receive thread once per process."""
        try:
    ...
    def set_auto_report_state(self, enable, forever=False):
        """Enable/disable periodic report frames from the controller board."""
        try:
            state1 = 0
            state2 = 0
            if enable:
    ...
    def set_beep(self, on_time):
        """Send buzzer command to the controller board."""
        try:
            if on_time < 0:
    ...
    def set_pwm_servo_all(self, angle_s1, angle_s2, angle_s3, angle_s4):
        """Set all four PWM servo angles in one serial frame."""
        try:
    ...
    def set_colorful_lamps(self, led_id, red, green, blue):
        """Set RGB lamp color for one LED or all LEDs."""
        try:
            id = int(led_id) & 0xff
            r = int(red) & 0xff
            g = int(green) & 0xff
    ...
    def set_colorful_effect(self, effect, speed=255, parm=255):
        """Start one of the board's built-in RGB effects."""
        try:
            eff = int(effect) & 0xff
            spe = int(speed) & 0xff
            par = int(parm) & 0xff
    ...
    def set_motor(self, speed_1, speed_2, speed_3, speed_4):
        """Set raw PWM speed for all four motors.

        This is the key method used by diff_driver_X3.py after it calculates
        the corrected wheel signs for your robot.
        """
        try:
    ...
    def set_car_run(self, state, speed, adjust=False):
        """Use the firmware's simple direction/speed motion command."""
        try:
    ...
    def set_car_motion(self, v_x, v_y, v_z):
        """Use the firmware's velocity motion command.

        X1/R2 drivers call this directly. The X3 driver currently uses
        set_motor() instead because its wheel signs were manually verified.
        """
        '''
        输入范围 input range: 
    ...
    def set_pid_param(self, kp, ki, kd, forever=False):
        """Set motor PID gains used by firmware velocity control."""
        try:
            state = 0
            if forever:
                state = 0x5F
    ...
    def set_car_type(self, car_type):
        """Tell the controller firmware which chassis kinematics to use."""
    ...
    def set_uart_servo(self, servo_id, pulse_value, run_time=500):
        """Command one UART servo by raw pulse value and motion time."""
        try:
    ...
    def set_uart_servo_angle(self, s_id, s_angle, run_time=500):
        """Command one arm servo by angle after converting it to pulse value."""
        try:
            if s_id == 1:
                if 0 <= s_angle <= 180:
    ...
    def set_uart_servo_id(self, servo_id):
        """Change the UART servo id. Use only with one servo connected."""
        try:
    ...
    def set_uart_servo_torque(self, enable):
        """Enable or disable UART servo torque output."""
        try:
            if enable > 0:
                on = 1
            else:
    ...
    def set_uart_servo_ctrl_enable(self, enable):
        """Enable or disable software-side arm command sending."""
        if enable:
            self.__arm_ctrl_enable = True
        else:
    ...
    def set_uart_servo_angle_array(self, angle_s=[90, 90, 90, 90, 90, 180], run_time=500):
        """Command all six arm servos using angle values."""
        try:
            if not self.__arm_ctrl_enable:
                return
    ...
    def set_uart_servo_offset(self, servo_id):
        """Ask firmware to store current servo offset/calibration."""
        try:
    ...
    def set_akm_default_angle(self, angle, forever=False):
        """Set the Ackermann steering servo neutral/default angle."""
        try:
    ...
    def set_akm_steering_angle(self, angle, ctrl_car=False):
        """Set Ackermann steering angle, optionally through car-control mode."""
        try:
    ...
    def reset_flash_value(self):
        """Reset firmware-saved settings in controller flash."""
        try:
    ...
    def reset_car_state(self):
        """Ask the controller to reset its runtime state."""
        try:
    ...
    def clear_auto_report_data(self):
        """Clear cached auto-report values in this Python object."""
        self.__battery_voltage = 0
        self.__ax = 0.0
        self.__ay = 0.0
        self.__az = 0.0
    ...
    def get_akm_default_angle(self):
        """Request and return the Ackermann default steering angle."""
    ...
    def get_uart_servo_value(self, servo_id):
        """Request raw pulse feedback for one UART servo."""
        try:
            if servo_id < 1 or servo_id > 250:
    ...
    def get_uart_servo_angle(self, s_id):
        """Request one arm servo angle in degrees."""
        try:
            angle = -1
    ...
    def get_uart_servo_angle_array(self):
        """Request all six arm servo angles."""
        try:
            # angle = [-1, -1, -1, -1, -1, -1]
            # for i in range(6):
    ...
    def get_accelerometer_data(self):
        """Return cached acceleration tuple (ax, ay, az)."""
    ...
    def get_gyroscope_data(self):
        """Return cached angular velocity tuple (gx, gy, gz)."""
    ...
    def get_magnetometer_data(self):
        """Return cached magnetic field tuple (mx, my, mz)."""
    ...
    def get_imu_attitude_data(self, ToAngle=True):
        """Return cached roll, pitch, yaw attitude from the controller."""
        if ToAngle:
            RtA = 57.2957795
    ...
    def get_motion_data(self):
        """Return cached chassis velocity tuple (vx, vy, vz)."""
        val_vx = self.__vx
        val_vy = self.__vy
        val_vz = self.__vz
    ...
    def get_battery_voltage(self):
        """Return cached battery voltage reported by the controller."""
    ...
    def get_motor_encoder(self):
        """Return cached encoder counts for motors M1-M4."""
    ...
    def get_motion_pid(self):
        """Request and return firmware motion PID gains."""
        self.__kp1 = 0
        self.__ki1 = 0
        self.__kd1 = 0
        self.__pid_index = 0
    ...
    def get_car_type_from_machine(self):
        """Request and return the car type currently stored on the board."""
    ...
    def get_version(self):
        """Request and return firmware version as major.minor."""
